...
def move_towards(current, target, max_distance_delta):
    ...
    res[~ended_mask] = (
        current[~ended_mask]
        + delta[~ended_mask] / dist[~ended_mask][:, None] * max_distance_delta
    )
    return res
...
def bloat(centroid, points):
    max_bloat = np.min(
        [centroid[0] + 1, 1 - centroid[0], centroid[1] + 1, 1 - centroid[1]]
    )
    ...
    for i in range(len(points)):
        ...
        if np.sign(a1) != np.sign(a2):
            target_pos = (
                centroid
                + normalize((ca + cb) / 2)
                * (np.linalg.norm(ca) + np.linalg.norm(ca))
                / 2
            )
...
def drift(centroids):
    if len(centroids) == 1:
        return np.array(
            [move_towards_vec(centroids[0, :], np.array([0, 0]), DRIFT_STEP)]
        )
    else:
        ...
        for i in range(len(centroids)):
            ...
            from_other[0, :] = (1 - t) * from_other[0, :] + t * normalize(-c) * dists[
                0
            ]  # combination of direction directly from wall and toward center
